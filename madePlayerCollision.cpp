...
madePlayerCollision::madePlayerCollision()
{
}
...
HRESULT madePlayerCollision::init()
{
	_map = new madeTestMapTool;
	_map->init();
...
void madePlayerCollision::release()
{
	SAFE_DELETE(_map);
}

void madePlayerCollision::update()
{
	if (KEYMANAGER->isStayKeyDown('A') && _rc.left > 0)
	{
		_direction = SECONDPLAYERDIRECTION_LEFT;
		playerMove();
	}
	if (KEYMANAGER->isStayKeyDown('D') && _rc.right < WINSIZEX) {
		_direction = SECONDPLAYERDIRECTION_RIGHT;
		playerMove();
	}
	if (KEYMANAGER->isStayKeyDown('W') && _rc.top > 0) {
		_direction = SECONDPLAYERDIRECTION_UP;
		playerMove();
	}
	if (KEYMANAGER->isStayKeyDown('S') && _rc.bottom < WINSIZEY) {
		_direction = SECONDPLAYERDIRECTION_DOWN;
		playerMove();
	}
}

void madePlayerCollision::render()
{
...
void madePlayerCollision::setPlayerPosition()
{
	sampleRc = RectMakeCenter(WINSIZEX / 2 + 200, WINSIZEY / 2, 96, 96);
	_rc = sampleRc;
	_x = _rc.left + (_rc.right - _rc.left) / 2;
	_y = _rc.top + (_rc.bottom - _rc.top) / 2;
}

void madePlayerCollision::playerMove()
{
	RECT rcCollision;
	...
	{
	case SECONDPLAYERDIRECTION_LEFT:
		_x -= _speed;
		...
		break;
	case SECONDPLAYERDIRECTION_RIGHT:
		_x += _speed;
		rcCollision = RectMakeCenter(_x, _y, 96, 96);
		_frameY = 3;
		_frameCount++;
	...
	case SECONDPLAYERDIRECTION_UP:
		...
		break;
	case SECONDPLAYERDIRECTION_DOWN:
		_y += _speed;
		rcCollision = RectMakeCenter(_x, _y, 96, 96);
		_frameY = 1;
		_frameCount++;
